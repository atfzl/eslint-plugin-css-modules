  ...
  create (context: Object) {
    ...
    */
    ...
    return {
      ...
      'Program:exit' () {
        /*
           Check if all classes defined in css/scss are used
        */
        _.forOwn(map, (o) => {
          const { classes, node } = o;

          // classNames with value not marked as true are unused
          const unusedClasses = Object.keys(_.omitBy(classes, null));

          if (!_.isEmpty(unusedClasses)) {
            context.report(node, `unused classes: ${unusedClasses.join(', ')}`);
          }
        });
      }
    };
