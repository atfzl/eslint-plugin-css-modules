...
export const getImportedFilePaths = (ast: gASTNode, parentFilePath: string) => {
  const filePaths = [];

  ast.traverse((node, index, parent) => {
    if (node.type === 'atrule') {
      const keyword = node.content.find(node => node.type === 'atkeyword').content[0].content;
      
      if (keyword !== 'import') return;

      const filePathNode = node.content.find(node => node.type === 'string');
      const filePath = filePathNode.content.replace(/^('|")/g, '').replace(/('|")$/, '');

      if (!filePath || filePath.startsWith('~')) return;

      const filePathWithExt = /\.(css|scss|less)$/.test(filePath) ? filePath : `${filePath}.${filePathNode.syntax}`;

      filePaths.push(path.resolve(path.dirname(parentFilePath), filePathWithExt));
    }
  });
