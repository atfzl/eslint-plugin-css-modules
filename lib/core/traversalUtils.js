...
export const eliminateGlobals = (ast: gASTNode) => {
  const hasGlobalIdent = fp.compose(
    fp.negate(fp.isEmpty),
    fp.filter({ type: 'ident', content: 'global' }),
    fp.get('content')
  );
  const hasGlobalAsFirstSelector = fp.compose(
    hasGlobalIdent,
    fp.find({ type: 'pseudoClass' }),
    // Only eliminate :global if it's the first selector
    fp.slice(0, 1),
    fp.get('content'),
    fp.find({ type: 'selector' }),
    fp.get('content'),
  ...
  ast.traverseByType('ruleset', (node, index, parent) => {
    if (hasGlobalAsFirstSelector(node)) {
      parent.removeChild(index);
    }
  });

  const getGlobalSelectors = fp.compose(
    fp.filter((node) => hasGlobalIdent(node.node)),
    fp.filter(node => node.node.type === 'pseudoClass'),
  );
  // Eliminate remaining :global selectors, but not the whole rule
  ast.traverseByType('selector', (node) => {
    const mappedContent = node.content.map((node, i) => ({ node, i }));
    const globalSelectors = getGlobalSelectors(mappedContent);
    if (globalSelectors) {
      for (let i = globalSelectors.length - 1; i >= 0; i--) {
        node.removeChild(globalSelectors[i].i);
      }
    }
  });
};
